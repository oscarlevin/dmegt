...
MAIN = $(SRC)/dmegt.ptx
MERGED = $(LOCALBUILD)/dmegt-merge.ptx

# These paths are subdirectories of
# a scratch directory
HTMLOUT    = $(SCRATCH)/docs
...
ww-merge:
	cd $(SCRATCH); \
	xsltproc --xinclude --stringparam webwork.extraction $(LOCALBUILD)/webwork-extraction.xml $(PTXXSL)/pretext-merge.xsl $(MAIN) > $(LOCALBUILD)/dmegt-merge.ptx
...
html: ww-merge
	...
	cp -a images $(HTMLOUT)
	install -d $(PTXUSR)
	install -b $(XSL)/dmegt-html.xsl $(PTXUSR)
	install -b $(XSL)/dmegt-common.xsl $(PTXUSR)
	cd $(HTMLOUT); \
	xsltproc -xinclude $(PTXUSR)/dmegt-html.xsl $(MERGED);

html-fresh: diagrams ww-extraction html

viewhtml:
	$(HTMLVIEWER) $(HTMLOUT)/dmegt.html &
...
#   This is the dmegt/Sage downloadable Annual Edition
# sage:
...
# 	cd $(PDFOUT); \
# 	xsltproc --xinclude $(MBUSR)/aata-sage.xsl $(SRC)/dmegt.ptx; \
...
latex: ww-merge
	...
	cp -a images $(PDFOUT)
	install -d $(PTXUSR)
	install -b $(XSL)/dmegt-latex.xsl $(PTXUSR)
	install -b $(XSL)/pretext-latex-dmoi.xsl $(PTXUSR)
	install -b $(XSL)/dmegt-common.xsl $(PTXUSR)
	cd $(PDFOUT); \
	xsltproc --xinclude $(PTXUSR)/dmegt-latex.xsl $(MERGED) > dmegt.tex;
...
print: latex
	cd $(PDFOUT); \
	$(ENGINE) dmegt.tex; $(ENGINE) dmegt.tex; \
	mv dmegt.pdf dmegt-print.pdf

	# View PDF from correct directory
viewpdf:
	$(PDFVIEWER) $(PDFOUT)/dmegt.pdf
	
viewprint:
	$(PDFVIEWER) $(PDFOUT)/dmegt-print.pdf
